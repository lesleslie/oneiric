...
1. `0a03d48` - Add cache key generation
1. `fc886fc` - Add fallback embedding generation
1. `2c7cdb0` - Add sentence-transformers integration with caching
1. `f010963` - Add integration tests
1. `0186163` - Apply ruff formatting
1. `f703a48` - Integrate EmbeddingService with OTelStorageAdapter
...
1. Add vector similarity search (Pgvector cosine similarity)
1. Implement trace correlation queries
1. Add SQL escape hatch for complex queries
...
1. **Fallback logic** - Ensures traces never fail to store

### Challenges Overcome

1. **Import resolution** - IDE warnings vs actual code (tests prove imports work)
1. **Integration test requirements** - Needs PostgreSQL (marked appropriately)
1. **Numpy array to database** - Convert to list() for JSONB storage
1. **Async/sync bridge** - Use asyncio.run() in tests
1. **Lazy loading** - Model loaded on first use (cache warming)

### Technical Decisions

1. **LRU cache vs manual cache** - functools.lru_cache simpler
1. **Fallback embedding** - Hash-based ensures determinism
1. **Embedding in \_flush_buffer** - Generated during DB write phase
1. **tolist() conversion** - Required for database JSONB storage
1. **Integration test markers** - Properly marked as integration/slow
