...
### Ownership Overview (Weeks 7-16)

| Stage | Window | Primary Owner | Key Deliverables |
|-------|--------|---------------|------------------|
| Documentation Push | Weeks 7-9 | Les (Docs/DevRel) | API docstrings ≥80%, architecture guide refresh, 10 runnable examples, CONTRIBUTING/AGENTS alignment |
| Production Hardening | Weeks 10-12 | Runtime/Infra owner (Les unless delegated) | Circuit breaker + retry/backoff helpers, perf baseline suite, observability tuning (logging sinks, OTel metrics) |
| Plugin & Entry-Point Protocol | Weeks 11-13 | Core platform owner | Entry-point discovery module, sample plugin package, migration notes for adapter authors |
| Final Polish & Release Prep | Weeks 13-16 | Shared (Les + reviewers) | Deployment docs (Docker/K8s), CI+release automation, community assets, security re-audit |
...
### Weeks 7-9: Documentation *(Owner: Les / DevRel)*

- Week 7: Drive docstring coverage to 80%+, refresh architecture diagrams, and publish AGENTS/CONTRIBUTING cross-links.
- Week 8: Ship 10 runnable examples (CLI demos, manifest recipes, orchestration walkthroughs) plus integrate them into `docs/`.
- Week 9: Perform API stability review, produce deprecation policy + type stubs, and schedule async doc review.

### Weeks 10-12: Production Hardening *(Owner: Runtime/Infra)*

- Week 10: Implement reusable circuit breaker + retry/backoff helpers (tie into remote loader + lifecycle swaps) and add coverage.
- Week 11: Run performance profiling, capture benchmarks, and land optimizations (resolver hot paths, watcher throughput).
- Week 12: Expand metrics/logging per `LOGGING_OBSERVABILITY_PLAN.md`, finalize monitoring guide, and expose config knobs in settings.

### Weeks 13-16: Final Polish *(Owner: Shared)*

- Week 13: Finish plugin entry-point discovery + capability negotiation (ties back to Phase 4 table) with sample adapter migration notes.
- Week 14: Produce Docker/K8s manifests, wire CI/CD deploy workflows, and validate orchestrator in containerized envs.
- Week 15: Ship community assets (updated CONTRIBUTING, at least 3 reference plugins), gather beta feedback.
- Week 16: Run security audit + load testing, publish release candidate + final v1.0.0 notes.
...
1. **Split CLI Monolith** - 1000-line `cli.py` needs modularization
1. **Extract Shared Utilities** - Duplicate `_maybe_await()` in 2 files
1. **Protocol-Based Interfaces** - Replace concrete class coupling with protocols

### Medium Priority (Weeks 10-12)

5. **Race Condition in Registry** - Add `threading.Lock()` or document single-threaded
1. **Unbounded Instance Cache** - Implement LRU eviction
1. **Secrets Cache Invalidation** - Add TTL or config reload hook
1. **Sequential Remote Registration** - Parallelize with `asyncio.gather()`
...
1. First-class hot-swapping (vs manual reload)
1. Explain API (vs implicit resolution)
1. CLI diagnostics (vs none)
1. Remote-native design

### ACB Still Better

1. Production maturity (31 releases vs greenfield)
1. Adapter ecosystem (60+ vs 3 reference plugins)
1. Full platform (implementations vs bridges)
1. Established community
1. FastBlocks + MCP integration
...
1. **Path Sanitization** (`loader.py:52-54`)
...
1. **HTTP Timeouts** (`loader.py:73, 234`)
...
1. **Verify with tests** (Weeks 3-6)
1. **Document for users** (Weeks 7-9)
1. **Harden for production** (Weeks 10-12)
1. **Launch with confidence** (Weeks 13-16)
