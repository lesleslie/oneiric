...
2) `register_pkg`-inferred priority: derive from caller path (e.g., `sites/*` > `splashstand` > `fastblocks` > `oneiric`) using a configurable precedence map/env var; fallback to path depth if unknown.
3) Adapter metadata `stack_level` (z-index): higher value wins inside the same package.
4) Registration order (last registered wins as final tie-breaker).
...
- Flow:
  ...
  3) Download artifact to a local cache dir; install as wheel/zip or add to `sys.path`.
  4) Register the package with an inferred priority (from manifest + stack_level) and merge into the resolver.
  5) Offline: use cached copy; fall back to local adapters if fetch/verify fails.
