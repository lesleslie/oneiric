...
2. Runtime watchers tests (10 tests)
   - SelectionWatcher triggering
   - Config file monitoring
   - Hot-swap automation
   - Error handling

3. Runtime health tests (5 tests)
   - Health snapshot generation
   - Runtime metrics tracking
   - Health check aggregation

4. Activity state tests (5 tests)
...
2. List/explain/status commands
3. Health/probe commands
4. Swap/pause/drain commands
5. Remote sync commands
6. JSON output validation
7. Demo mode functionality

**Integration Tests (~20 tests)**
1. End-to-end workflows (10 tests)
2. Multi-domain orchestration (5 tests)
...
2. **Test-Driven API Discovery:** Used tests to validate actual behavior (Resolver.list_active vs list_all)
3. **Cryptographic Testing:** Generated real keypairs for authentic signature verification
4. **End-to-End Validation:** Tested full sync flow from manifest → candidates
5. **Pydantic Model Testing:** Validated required fields and serialization
6. **Mock HTTP Requests:** Used unittest.mock for HTTP artifact fetching
