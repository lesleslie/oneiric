...
1. **Allowlist/Denylist for Sources** (RESOLUTION_LAYER_SPEC.md line 83)
...
1. **Wheel/Zip Installation** (NEW_ARCH_SPEC.md line 48, RESOLUTION_LAYER_SPEC.md line 63)
...
1. **Entry Point Discovery** (RESOLUTION_LAYER_SPEC.md line 41, GRAND_IMPLEMENTATION_PLAN.md line 49)
...
1. **Capability Tags** (NEW_ARCH_SPEC.md line 89)
...
1. **Runtime Health Snapshots** (BUILD_PROGRESS.md mentions, not in original specs)
...
1. **Telemetry Caching** (remote/telemetry.py)
...
1. **Comprehensive CLI** (Goes beyond spec requirements)
...
1. **Lifecycle Swap Logic** (Should have ~20 test cases)
...
1. **Remote Manifest Security** (Should have ~10 test cases)
...
1. **Config Watchers** (Should have ~8 test cases)
...
1. Remote manifest refresh with network failures
1. Health check failures during production swap
1. Pause/drain state transitions during active swaps
1. Memory leaks from instance caching
1. Concurrent resolver access (thread safety unclear)
...
1. **Source Allowlist/Denylist** (HIGH)
...
1. **TLS Certificate Pinning** (MEDIUM)
...
1. Technical debt not being tracked inline (risky)
1. Issues externalized to docs/issues (unknown)
...
1. **TaskGroup Result Collection Assumes Success**
...
1. **Sequential Remote Entry Registration**
...
1. **Circuit Breaker for Remote Sync** (MEDIUM - Phase 5 deferred)
...
1. **Retry Logic with Exponential Backoff** (LOW - Phase 5 deferred)
...
1. No input validation on remote manifests
1. Thread safety unknown (no tests, no documentation)
...
1. **Comprehensive Feature Set**: 5 domains, remote manifests, CLI, observability all working
1. **Modern Python**: Strong type hints, async/await, Python 3.14 idioms
1. **Operational Tooling**: CLI with health checks, diagnostics, status snapshots
1. **Structured Observability**: OpenTelemetry spans, structured logs, metrics

### 9.4 Top 5 Weaknesses

1. **Zero Test Coverage**: Showstopper for production deployment
1. **Critical Security Vulnerabilities**: Arbitrary code execution, path traversal
1. **Missing Signature Verification**: Can't trust remote manifests in hostile networks
1. **Inadequate Input Validation**: Remote manifests trusted too much
1. **Insufficient Documentation**: No docstrings, unclear error messages, missing architecture docs
...
1. **Add HTTP timeouts** (2 hours, HIGH ROI) - Prevents hangs
1. **Fix path traversal** (2 hours, HIGH ROI) - Blocks directory escape
1. **Add thread safety docs** (1 day, MEDIUM ROI) - Clarifies usage constraints
...
1. **Complete absence of testing**, making correctness unverifiable
1. **Missing spec-required features** like signature verification
