...
## status: draft role: canonical date: 2026-01-02 last_reviewed: 2026-07-17 superseded_by: null blocks_on: [] topic: mcp-design
...
1. **Oneiric Standard:** Follow Oneiric CLI factory patterns
1. **No Legacy Support:** Remove all legacy CLI flags and patterns
1. **Health First:** Prioritize health and status reporting
1. **Instance Isolation:** Support `--instance-id` for multi-instance deployments
...
|---------|-------------|----------|-------|
| `start` | Start the MCP server | ✅ Yes | Writes PID file, initializes runtime snapshots |
| `stop` | Stop the running server | ✅ Yes | Reads PID file, graceful shutdown |
| `restart` | Restart the server | ✅ Yes | Stop + Start sequence |
| `status` | Show server status | ✅ Yes | Reads runtime snapshots + PID |
| `health` | Health check endpoint | ✅ Yes | Reads cached health snapshot |
| `health --probe` | Live health probe | ✅ Yes | Performs live health checks |

### Optional Commands (Project-Specific)

| Command | Description | Required | Notes |
|---------|-------------|----------|-------|
| `config show` | Display current configuration | ❌ No | Use Oneiric config patterns |
| `config validate` | Validate configuration | ❌ No | Use Pydantic validation |
| `version` | Show version information | ❌ No | Use `--version` flag |
| `help` | Show help information | ❌ No | Auto-generated by CLI factory |

______________________________________________________________________

## 🔄 Project-Specific CLI Migrations
...
1. ✅ Create `mailgun_mcp/__main__.py` with CLI factory
1. ✅ Add console script entry point in `pyproject.toml`
1. ✅ Implement `MailgunMCPServer` with Oneiric lifecycle
1. ✅ Add health check endpoints
1. ✅ Implement runtime snapshot management
1. ✅ Remove all legacy CLI patterns
...
1. ⏳ Create `unifi_mcp/__main__.py` with CLI factory
1. ⏳ Add console script entry point in `pyproject.toml`
1. ⏳ Implement `UniFiMCPServer` with Oneiric lifecycle
1. ⏳ Add health check endpoints
1. ⏳ Implement runtime snapshot management
1. ⏳ Remove all legacy CLI patterns
...
1. ⏳ Update `opera_cloud_mcp/__main__.py` with CLI factory
1. ⏳ Modify console script entry point
1. ⏳ Implement `OperaCloudMCPServer` with Oneiric lifecycle
1. ⏳ Add health check endpoints
1. ⏳ Implement runtime snapshot management
1. ⏳ Remove all legacy CLI patterns
...
1. ⏳ Update `raindropio_mcp/__main__.py` with CLI factory
1. ⏳ Modify console script entry point
1. ⏳ Implement `RaindropMCPServer` with Oneiric lifecycle
1. ⏳ Add health check endpoints
1. ⏳ Implement runtime snapshot management
1. ⏳ Remove all legacy CLI patterns
...
1. ⏳ Make migration approach decision (Python rewrite vs Node.js adapter)
1. ⏳ Implement chosen approach
1. ⏳ Add Oneiric CLI factory (Python or Node.js)
1. ⏳ Implement lifecycle management
1. ⏳ Add health check endpoints
1. ⏳ Implement runtime snapshot management
1. ⏳ Preserve WebSocket functionality
...
- Node.js ↔ Oneiric bridge complexity
...
└── instance-1/                 # Multi-instance support
    ├── server.pid
    ├── runtime_health.json
...
1. **No Legacy Flags:** Remove all legacy `--start`, `--stop` flags
1. **Instance Isolation:** Support `--instance-id <id>` for multi-instance
1. **Exit Codes:** Follow standard exit codes (0=success, 1=error)
1. **Help Output:** Auto-generated by CLI factory
...
1. **Multi-instance:** `.oneiric_cache/<instance-id>/`
1. **PID File:** `server.pid` with process ID
1. **Health Snapshot:** `runtime_health.json` with mcp-common schema
1. **Telemetry:** `runtime_telemetry.json` with metrics
...
- ✅ `restart` command functionality
- ✅ `status` command output
- ✅ `health` command output
- ✅ `health --probe` live checks
- ✅ Error handling and exit codes
- ✅ Instance isolation (`--instance-id`)
...
- [ ] ✅ Create CLI command mapping guide (this document)
- [ ] ⏳ Implement Oneiric CLI factory
- [ ] ⏳ Create server class with lifecycle hooks
- [ ] ⏳ Implement configuration class
- [ ] ⏳ Add console script entry point
- [ ] ⏳ Implement health check endpoints
- [ ] ⏳ Add runtime cache management
- [ ] ⏳ Remove legacy CLI patterns
...
- [ ] ⏳ Standardize health schema
- [ ] ⏳ Standardize runtime cache
...
1. **Rollback Procedures:** Tested rollback for each project
1. **Incremental Testing:** Test each command individually
1. **User Communication:** Clear migration guides and examples
1. **Performance Monitoring:** Benchmark critical operations
...
- [ ] ✅ All projects use Oneiric CLI factory
- [ ] ✅ Standardized command structure across all projects
- [ ] ✅ Health schema compliance (mcp-common.health)
- [ ] ✅ Runtime cache implementation
- [ ] ✅ Instance isolation support
- [ ] ✅ Zero legacy CLI patterns
- [ ] ✅ Comprehensive test coverage
- [ ] ✅ Performance metrics maintained

### User Success Metrics

- [ ] ✅ Clear CLI command mapping documentation
...
1. **Complete CLI Mapping:**
   ...
   - [ ] ✅ Document mailgun-mcp CLI mapping
   - [ ] ⏳ Document unifi-mcp CLI mapping
   - [ ] ⏳ Document opera-cloud-mcp CLI mapping
   - [ ] ⏳ Document raindropio-mcp CLI mapping
   - [ ] ⏳ Document excalidraw-mcp CLI mapping

1. **Begin Implementation:**

   - [ ] ⏳ Start with mailgun-mcp (simplest)
...
1. **Standardization:**
   ...
   - [ ] ⏳ Ensure consistent CLI patterns across projects
   - [ ] ⏳ Validate compatibility contract compliance
   - [ ] ⏳ Review health schema implementations
