...
2. **`activity_store` vs `_activity_store`:**
   - Tests used `bridge.activity_store`
   - Actual API: `bridge._activity_store` (private)

3. **`health_snapshot_path` vs `_health_path`:**
...
2. ✅ Creates 5 selection watchers on init
3. ✅ All bridges share same `DomainActivityStore`
4. ✅ `start()` starts all watchers
5. ✅ `start()` with manifest_url triggers initial sync
6. ✅ `start()` with refresh_interval creates sync loop task
7. ✅ `stop()` cancels all watchers
8. ✅ `stop()` cancels remote sync task
9. ✅ Works as async context manager
10. ✅ Health snapshot created/updated on start/stop

### Remote Sync
1. ✅ Calls `sync_remote_manifest()` with correct args
2. ✅ Updates health snapshot on success with registration stats
3. ✅ Updates health snapshot on error with error message
4. ✅ Persists per-domain registration counts
5. ✅ Persists skipped entry count

### Watcher Lifecycle
1. ✅ Initializes with bridge and config loaders
2. ✅ Accepts custom poll interval
3. ✅ `start()` creates polling task
4. ✅ `stop()` cancels task and waits for completion
5. ✅ Works as async context manager
6. ✅ `stop()` without `start()` is safe (no-op)
7. ✅ `start()` on running watcher raises RuntimeError

### Activity State Integration
1. ✅ Watcher respects paused state (no swaps when paused)
2. ✅ Watcher respects draining state (delays swaps when draining)
3. ✅ Activity store properly created and shared

### Health Snapshot Persistence
1. ✅ Loads from existing JSON
2. ✅ Returns defaults if file missing
3. ✅ Returns defaults if JSON invalid
4. ✅ Handles partial JSON data
5. ✅ Writes create parent directories
6. ✅ Atomic write (no temp files left behind)
7. ✅ Includes updated timestamp
8. ✅ Roundtrip preserves all data
...
2. **Integration Tests** (~20 tests) - Week 6
