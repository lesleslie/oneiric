...
2) `register_pkg` inferred priority (caller path mapped to stack order, e.g., sites > splashstand > fastblocks > oneiric).
3) Metadata `stack_level` (z-index) on the candidate.
4) Registration order (last write wins as final tie-breaker).
...
- Flow:
  ...
  3) Download artifact to cache; install/import or add to `sys.path`.
  4) Register as a candidate with manifest-provided `stack_level` and source priority.
  5) Offline fallback: use cached artifact; if missing, fall back to local candidates.
...
2) Migrate adapters to use resolver (keep existing behavior; add stack_level).
3) Add hot-swap for adapters; integrate config/manifest diff watcher.
4) Generalize to services/tasks/events/workflows behind the same resolver.
5) Add observability surfaces (CLI/table, debug logs).
6) Add remote manifest support (start with signed wheel/zip over HTTPS/S3/GCS).
7) Optional: entry-point discovery, OCI artifacts, MCP registry integration.
