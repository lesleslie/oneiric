...
2. `resolve()` - Consistent reads of active candidates
3. `list_active()` - Snapshot of active candidates for domain
4. `list_shadowed()` - Snapshot of shadowed candidates for domain
5. `explain()` - Consistent scoring/ranking for resolution explanation
...
2. **Update Unified Implementation Plan** (30 minutes)
...
2. **Core Lifecycle Test Suite** (3-4 days)
...
2. **Comprehensive Testing:** 10 tests covering all concurrency scenarios
3. **Single Lock Simplicity:** Easy to reason about, no lock ordering issues
4. **Fast Implementation:** Completed in < 1 hour (target: 1 day)
...
2. **Lock Granularity Decision:** Chose simplicity over maximum performance
3. **Reentrant vs. Regular Lock:** Analyzed call patterns to determine need
