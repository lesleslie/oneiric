"""Tests for oneiric/adapters/mcp_pusher.py — MCPAdapterPusher, push_adapters_on_startup, main."""
...
from oneiric.adapters.mcp_pusher import (
    MCPAdapterPusher,
    main,
...
class TestPusherInit:
    def test_init_defaults(self):
        p = MCPAdapterPusher()
        assert p.dhara_url == "http://127.0.0.1:8683"
        assert p.timeout == 30.0

    def test_init_custom_url(self):
        p = MCPAdapterPusher(dhara_url="http://example.com:9999")
        assert p.dhara_url == "http://example.com:9999"
    ...
    def test_init_custom_timeout(self):
        p = MCPAdapterPusher(timeout=60.0)
