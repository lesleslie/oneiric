...
from oneiric.adapters.druva_pusher import DruvaAdapterPusher, push_adapters_on_startup
...
def _make_pusher(response: dict[str, Any] | None = None, raise_http: bool = False) -> tuple[DruvaAdapterPusher, list[dict[str, Any]]]:
    calls: list[dict[str, Any]] = []
    pusher = DruvaAdapterPusher()
...
# ---------------------------------------------------------------------------
# DruvaAdapterPusher tests
# ---------------------------------------------------------------------------


def test_pusher_init_defaults() -> None:
    """__init__() sets druva_url and timeout and creates httpx.Client."""
    pusher = DruvaAdapterPusher()
    assert pusher.druva_url == "http://127.0.0.1:8683"
    assert pusher.timeout == 30.0
    pusher.close()


def test_pusher_init_strips_trailing_slash() -> None:
    """__init__() strips trailing slash from druva_url."""
    pusher = DruvaAdapterPusher(druva_url="http://localhost:8683/")
    assert pusher.druva_url == "http://localhost:8683"
    pusher.close()
...
def test_push_single_adapter_http_status_error() -> None:
    ...
    pusher.client.post = lambda url, json: _FakeResponse({"detail": "server error"}, status=500)  # type: ignore[method-assign]
    meta = _make_metadata()
    result = pusher._push_single_adapter(meta)
    assert result["success"] is False
...
def test_push_builtin_adapters_exception_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(DruvaAdapterPusher, "_push_single_adapter", explode)
    adapters = [_make_metadata()]
    results = pusher.push_builtin_adapters(adapters)
    assert results["errors"] == 1
...
def test_push_adapters_on_startup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    meta = _make_metadata()
    monkeypatch.setattr(
        "oneiric.adapters.druva_pusher.DruvaAdapterPusher._push_single_adapter",
        lambda self, m: {"success": True},
    )
    with patch("oneiric.adapters.bootstrap.builtin_adapter_metadata", return_value=[meta]):
...
def test_main_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from oneiric.adapters.druva_pusher import main

    monkeypatch.setattr(
        "oneiric.adapters.druva_pusher.push_adapters_on_startup",
        lambda **_: {"success": 2, "total": 2, "errors": 0, "details": []},
    )
    monkeypatch.setattr("sys.argv", ["druva_pusher"])
    result = main()
    assert result == 0


def test_main_with_errors(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    monkeypatch.setattr("sys.argv", ["druva_pusher", "--druva-url", "http://test:8683"])
    result = main()
    assert result == 1
