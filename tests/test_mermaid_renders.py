...
Wraps `oneiric.tools.mermaid_validator.renderer.find_broken_mermaid_blocks`,
...
If this test fails, run `python -c "from oneiric.tools.mermaid_validator.renderer
import find_broken_mermaid_blocks; [print(e) for e in find_broken_mermaid_blocks()]"`
...
from oneiric.tools.mermaid_validator.renderer import (
...
def test_extract_mermaid_blocks_finds_expected_count() -> None:
    ...
    The docs/ directory has dozens of fenced mermaid blocks. If this fails,
    the extractor is silently broken (e.g., regex changed).
    """
    repo_root = Path(__file__).resolve().parent.parent
    blocks = extract_mermaid_blocks(repo_root / "docs" / "RESOLUTION_LAYER_SPEC.md")
    assert len(blocks) >= 1, "expected at least one mermaid block in RESOLUTION_LAYER_SPEC.md"
