...
2. **CLI Event Emission Exposure** (`oneiric/cli.py`)
...
3. **Event Dispatcher Logging** (`oneiric/runtime/events.py`)
...
2. **Oneiric's Lifecycle System** (`oneiric/core/lifecycle.py:277`)
   - Logs `swap-complete` events when provider swaps are completed
   - Includes domain, key, and provider information

3. **Crackerjack's Logging System** (`crackerjack/services/logging.py`)
...
2. **Crackerjack Event System**: Crackerjack's logging system outputs Oneiric events to console
...
2. **Crackerjack Integration** (`crackerjack/runtime/oneiric_workflow.py`):
...
2. **Test Crackerjack Integration**
   ```bash
...
2. **Add CLI Flag**: Provide `--suppress-events` option for users
3. **Enhance Crackerjack Integration**: Use context-aware logging
4. **Document Logging Options**: Update README with logging configuration examples
5. **Add Integration Tests**: Verify event suppression works in all scenarios
