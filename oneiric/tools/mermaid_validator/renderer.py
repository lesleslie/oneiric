...
# `rich` is optional — only required by `print_errors` (CLI pretty-printing).
# The pytest guard uses pytest.fail() and never touches this, so we import
# lazily to avoid a hard dep on rich for users who only run the validator.
console = None
...
# `node_modules/jsdom/` installed in the oneiric repo by `npm install`
...
def _locate_mermaid_core() -> Path | None:
    ...
    1. ``ONEIRIC_MERMAID_CORE`` env var (operator-pinned). Must pass
    ...
    env_override = os.environ.get("ONEIRIC_MERMAID_CORE")
    if env_override:
        path = Path(env_override).resolve()
        if _is_trusted_mermaid_path(path):
            return path
        raise RuntimeError(
            f"ONEIRIC_MERMAID_CORE={env_override} is not under a "
...
def _locate_jsdom() -> Path | None:
    ...
    Looks for the `node_modules/jsdom/` directory under the oneiric
    ...
    Override via ``ONEIRIC_JSDOM`` env var for CI where the
    oneiric repo lives at a different path.
    """
    env_override = os.environ.get("ONEIRIC_JSDOM")
    if env_override:
        path = Path(env_override).resolve()
        if path.is_file():
            return path
        raise RuntimeError(
            f"ONEIRIC_JSDOM={env_override} does not exist or is not a file"
        )

    # Walk up from this file to find the oneiric repo root.
...
def validate_mermaid_blocks(
    ...
    passed. The Node.js runner is `oneiric/tools/mermaid_validator/validate_mermaid.mjs`,
    which uses `mermaid.parse()` (lexer-only, no chrome needed).
    """
    if not blocks:
        return []

    runner, mermaid_core, jsdom = _resolve_mermaid_runtime()
    payload = _encode_blocks(blocks)
    completed = _run_mermaid_subprocess(
        runner, mermaid_core, jsdom, payload, timeout, len(blocks)
    )
    return _parse_mermaid_results(completed)


def _resolve_mermaid_runtime() -> tuple[Path, Path, Path]:
    """Locate the runner script, mermaid core, and jsdom; raise if any missing."""
    ...
    if not jsdom:
        raise RuntimeError(
            ...
            "run `npm install` in the oneiric repo to install the "
            "wave-11 dev dep, or set ONEIRIC_JSDOM to its absolute path"
        )

    return runner, mermaid_core, jsdom


def _encode_blocks(blocks: list[MermaidBlock]) -> str:
    """Serialize blocks to the JSON payload the Node.js runner expects."""
    return json.dumps(
...
def _run_mermaid_subprocess(
    ...
    """Invoke the Node.js validator; wrap missing-node / timeout as RuntimeError."""
    try:
        ...
        raise RuntimeError(
            f"validate_mermaid.mjs timed out after {timeout}s on {block_count} blocks"
        ) from e


def _parse_mermaid_results(
    completed: subprocess.CompletedProcess,
) -> list[MermaidValidationError]:
    """Translate the validator's stdout into a list of MermaidValidationError."""
...
def print_errors(errors: list[MermaidValidationError]) -> None:
    """Pretty-print the broken blocks for the oneiric mermaid CI guard."""
    # Lazy import: rich is only needed for CLI pretty-printing.
